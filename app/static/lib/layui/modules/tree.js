                ...
                entryDiv = $(['<div data-id="' + item.id + '" class="layui-tree-set' + (item.spread ? " layui-tree-spread" : "") + (item.checked ? " layui-tree-checkedFirst" : "") + '">'
                    ...
                    , function () {
                        if (options.isJump && item.href) {
                            return '<a href="' + item.href + '" target="_blank" class="' + ELEM_TEXT + '">' + (item.head ??"") + (item.title || item.label || options.text.defaultNodeName) + '</a>';
                        } else {
                            return '<span class="' + ELEM_TEXT + (item.disabled ? ' ' + DISABLED : '') + '">' + (item.head ??"") + (item.title || item.label || options.text.defaultNodeName) + '</span>';
                        }
                    }()
                    , '</div>'
                    ...
                    , function () {
                        ...
                        var editIcon = {
                            ...
                            , move_up: '<i class="layui-icon layui-icon-up" data-type="move_up"></i>'
                        ...
                        let edit = item.edit ?? options.edit
                        if (typeof edit === 'object') {
                            layui.each(edit, function (i, val) {
                ...
                layer.confirm('确认删除该节点 "<span style="color: #999;">' + (item.title || '') + '</span>" 吗？', function (index) {
                    ...
                    if (elem.siblings('.' + ELEM_SET).children('.' + ELEM_ENTRY)[0]) {
                        ...
                        if (options.showLine) {
                            ...
                            prevDiv.find('.' + ICON_CLICK).children('.layui-icon').removeClass(ICON_SUB);
                            //父节点所在层添加延伸线
