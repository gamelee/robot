    ...
    Class.pt.openLayer = function () {
        ...
        layer.setTop = function (layero) {
            let setZindex = function () {
                if (layero.data('maxminStatus') === 'min') return
                layer.zIndex++;
...
// 最小化
    layer.min = function (index, options) {
        ...
        if (!hasMinLeft) {
            ...
            left = (win.width() - 181 - left) + 'px'
            if (parseFloat(left) < 0) left = 0;
        }

        // 是否堆叠在左下角
        if (options.minStack) {
            settings.left = left;
            settings.zIndex = 9999999;
...
//仿系统prompt
    layer.prompt = function (options, yes) {
        ...
        var prompt,
            content = options.formType == 2 ? '<textarea class="layui-prompt layui-layer-input"' + style + placeholder + '></textarea>' : function () {
                return '<input type="' + (options.formType == 1 ? 'password' : 'text') + '" class="layui-prompt layui-layer-input"' + placeholder + '>';
            }();
