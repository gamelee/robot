;!function (win) {
    ...
    // 识别预先可能定义的指定全局对象
    let GLOBAL = win.LAYUI_GLOBAL || {};

    // 获取 layui 所在目录
    let getPath = function () {
        let jsPath = doc?.currentScript?.src || function () {
            let js = doc.scripts;
    ...
    function get_func_args_name(func) {
        let args = func.toString().match(/[\s\S]*?\(([^)]*)\)/)
    ...
    //定义模块
    Layui.prototype.define2 = function (factory) {
        let that = this
        if (typeof factory !== 'function') {
            return
        }
        let deps = get_func_args_name(factory).map(name => name === '$' ? 'jquery' : name)
        let callback = function () {
            let set_app = function (mod) {
                layui[mod.name] = mod
                config.status[mod.name] = true
            }

            let fn = function () {
                let args = []
                deps.forEach(function (name) {
                    args.push(layui[name])
                })
                let mod = factory(...args)
                set_app(mod)
                return mod.name
            }
            config.callback[fn()] = fn
            return this
        };
