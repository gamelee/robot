...
func (c *chrome) attachPage(args ...string) (err error) {
	for method, arg := range map[string]h{
		"Page.enable":          nil,
		...
		if _, err = c.send(method, arg); err != nil {
			c.kill()
			c.cmd.Wait()
			return err
		}
...
func (c *chrome) connect(url string) (err error) {

	defer func() {
		log.Println("cdp server started at", url, err)
	}()
	c.ws, err = websocket.Dial(url, "", "http://127.0.0.1")
	if err != nil {
		c.kill()
		return err
	}
